...
def add_jobs(job_patterns, matcher):
    for occupation in job_patterns:
        occupation_tokens = [{'LOWER': {'in': word.lower().split()}} for word in occupation.split()]
        matcher.add('JOB_TITLE', [occupation_tokens])

def extract_company_names(resume_text, stop_sections=("interests", "awards", "education", "projects", "personal projects", "skills")):
    doc = nlp(resume_text)
    company_names = []
